...
| **Cost** | **1.5×** — you issue two queries instead of one. That is the floor, it is unavoidable, and it is the whole story unless you add a reranker. |
...
1. **10,000 documents is not 2.4 TB.** The distortion measured here is scale-invariant in form but
   this corpus is small, uniform in document length, and English-only. See [§10](#10-threats-to-validity).
2. **The judge is a language model, not a human assessor.** Mitigated by a full second-judge
   re-grade ([§9.4](#94-two-judges)), not eliminated.
3. **One strategy is systematically under-credited.** `agentic-planned` retrieves more broadly than
   ...
   lower bound. See [§8.3](#83-holes-in-the-judgments).
...
- that the query-cost floor of splitting is two queries per user query, and the latency floor is
  below 1× because fan-out is concurrent.
...
indexing cost. Several of these are discussed in [§10](#10-threats-to-validity) and
[§11](#11-what-we-did-not-measure) but none are measured.
...
data is not a project anyone wants. It is measured in [§8.4](#84-splitting-to-scale).
...
| --- | ---: | ---: | :---: | ---: | ---: | ---: | --- |
| `global-bm25` | **0.634** | +0.096 | [+0.064, +0.128] | <0.0001 | $0.16 | 62 ms | best — but see [§9.1](#91-the-headline-that-did-not-survive) |
...
| Split + rank fusion | 0.86× | 1.5× | 0.94× |
...
- **Cost has a hard floor of 1.5×** that no technique avoids, because two indexes means two queries.
  Above that floor, every increment is buying a model, not fixing the split.
- **Latency is free — until it isn't.** Splitting alone is *faster*. Adding a model costs one to two
  orders of magnitude.

### 8.2 Where the options actually sit
...
## 9. Experiments that could have falsified this

Every claim above is paired with an experiment designed to break it. Two succeeded.

### 9.1 The headline that did not survive
...
### 9.5 Exhaustive nearest-neighbour
...
Mitigated by the second-judge re-grade (§9.4) but not eliminated — a bias shared across that family
would be invisible to the check.

**Internal.** Pooling bias is the dominant threat and it is not fully closed: `agentic-planned`
remains at 12.1% Hole@10 (§8.3). Candidate budget is equalized everywhere except agentic retrieval,
...
flowchart TD
    ...
    A -->|No| C{"What kind of<br/>retrieval?"}
    C -->|Vector only| D["<b>Merge on raw score.</b><br/>Costs exactly nothing."]
    C -->|Hybrid| E["<b>Split the legs.</b><br/>Never re-fuse a fused score."]
    C -->|Keyword| F{"Already paying<br/>for reranking?"}
    F -->|Yes| G["<b>Merge on rerankerScore.</b><br/>Parity. Don't add a second pass."]
    F -->|No| H["<b>Build the statistics sidecar.</b><br/>Merge on corrected scores.<br/>Free at query time."]
```
...
2. **Merge on scores, corrected.** One offline pass, nothing at query time.
3. **Decompose hybrid queries into their legs.** A hybrid score is already fused. The subscores come
   back free on the same response.
4. **Apply the right repair to the right signal.** IDF correction on a hybrid score makes things
   *worse* (−0.125).
5. **If you already pay for the semantic ranker, merge on `rerankerScore` and stop.** A second
   reranking pass is measurably identical and twice the price.
6. **If your workload is vector-only, do nothing.** Splitting is free.
7. **Do not use agentic retrieval's free mode as a merge engine.** Zero tokens, worst relevance in
   the study.
...
| --- | --- | --- |
| Keyword, cost-sensitive | **Pattern 1**, sidecar or client-side BM25 | parity to +0.013, 1.5× cost, 0.94× latency |
